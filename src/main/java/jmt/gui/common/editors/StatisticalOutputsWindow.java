...
public class StatisticalOutputsWindow extends JMTFrame {
	...
	public StatisticalOutputsWindow(MeasureDefinition measureDefinition, int measureIndex) {
		...
		try {
			...
			logger.error("Verbose CSV file not found for measure.", e);
		}
	}
	...
	public void initGUI() {
		...
		measuresPanel.setBorder(new TitledBorder(new EtchedBorder(), "Statistics"));
		// MEAN
		...
		pivotPanel.add(Box.createRigidArea(new Dimension((int)(0 * CommonConstants.widthScaling), (int)(15 * CommonConstants.heightScaling))));
		pivotPanel.add(momentsPanel);
		pivotPanel.add(Box.createRigidArea(new Dimension((int)(0 * CommonConstants.widthScaling), (int)(15 * CommonConstants.heightScaling))));
		pivotPanel.add(boundsPanel);
		pivotPanel.add(Box.createRigidArea(new Dimension((int)(0 * CommonConstants.widthScaling), (int)(15 * CommonConstants.heightScaling))));
		pivotPanel.add(sampleChooser);
		pivotPanel.add(Box.createRigidArea(new Dimension((int)(0 * CommonConstants.widthScaling), (int)(30 * CommonConstants.heightScaling))));
		...
		if (inverseMeasure) {
			JLabel warningLabel = new JLabel("<html>Please note that statistics for Arrival Rate are computed from inter-arrival times while those for Throughput, Drop Rate and System Power indices are based on inter-departure times.</html>");
	...
	private void updateGraph() {
		...
		if (type != TYPE_NONE && statGraph != null && statGraph.isDistributionAvailable()) {
			...
			graph.setMinimumSize(new Dimension((int)(200 * CommonConstants.widthScaling), (int)(150 * CommonConstants.heightScaling)));
			graph.setPreferredSize(new Dimension((int)(300 * CommonConstants.widthScaling), (int)(250 * CommonConstants.heightScaling)));
	...
	private void showSaveChartWindow() {
		...
		if (returnCode == JFileChooser.APPROVE_OPTION && chooser.getSelectedFile() != null) {
			...
			if (selectedFile.exists()) {
				int response = JOptionPane.showConfirmDialog(null, "Overwrite the CSV file: " + selectedFile.getAbsolutePath() + "?", "Confirm overwrite",
	 ...
	 */
	private void saveChartData(File targetFile, DistributionType type) {
		CSVLogger logger = JSimLoggerFactory.getCSVLogger(targetFile, new String[] {CSV_COLUMN_INDEX, CSV_COLUMN_START, CSV_COLUMN_END, CSV_COLUMN_VALUE}, false, 
		...
		for (int i = 0; i < values.length; i++) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "Failed to write the CSV file: " + targetFile.getAbsolutePath());
				break;
			}
		}
		try {
