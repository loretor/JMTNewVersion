...
public class CoMoMBasis extends Basis {
	...
	private void generate() {
		...
		for (int i = 1; i <= M; i++) {
			for (int j = 0; j < R - 1; j++) {
				for (int k = current_list_end - MiscMathsFunctions.binomialCoefficient(R - 1 - j + i - 2, i - 1);
			...
			current_list_end += MiscMathsFunctions.binomialCoefficient(R - 1 + i - 1, i );
		}
	}
	...
	@Override
	public void setSize() {
		size  = (M + 1)* MiscMathsFunctions.binomialCoefficient(M + R - 1, M);
	}	
