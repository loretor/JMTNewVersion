...
public class MoMSolver extends QNSolver {
	...
	private void generateABU(PopulationVector N, int s) throws InternalErrorException {
		...
		int limit = (int) ((float) R / M * MiscMathsFunctions.binomialCoefficient(M + R - 1, R) - 1);
	...
	private void initialise(int nThreads) {
		...
		int R = qnm.R, M = qnm.M;
		sz = MiscMathsFunctions.binomialCoefficient(M + R, R);
		matrixSize = sz * R;
		...
		try {
			...
			for (r = 1; r <= R; r++) {
				...
				for (Nr = N0r; Nr <= N.get(r - 1) - 1; Nr++) {
					mulTimer.start();
					BigRational[] sysB = MiscMathsFunctions.matrixVectorMultiply(B, curG);
					mulTimer.pause();
					s.goToULevel(Nr - 1);
					curG = solve(s, sysB);
					Ntot++;
